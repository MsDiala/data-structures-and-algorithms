...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const sum = arr.reduce((sumSoFar, value, index) => {
    return sumSoFar = sumSoFar + 1;
  }, 0);
  return sum;
...
const returnNames = (arr) => {
  // Solution code here...
  let answer = arr.reduce((characters, value, index) => {
...
------------------------------------------------------------------------------------------------ */
  ...
  // Solution code here...
  const reversedString = (str) => str.split('').reduce((reverse, characters) => characters + reverse, '');
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  const newArr = arr.reduce((sumSoFar, profile) => {
    if (profile.children) {
      for (let child = 0; child < profile.children.length; child++) {
        sumSoFar++;
      }
    }
    return sumSoFar;
...
const calculateAverage = (arr) => {
  // Solution code here...
  let countAndSum = arr.reduce((accumulator, value) => {
    accumulator.count = accumulator.count + 1;
    accumulator.sum = accumulator.sum + value;
    return accumulator;
  }, { count: 0, sum: 0, });
  let average = countAndSum.sum / countAndSum.count;
  return average;
};
